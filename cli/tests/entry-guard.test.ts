 ...
 * `npm install` materialises `bin/fg` as a SYMLINK, Node reports the symlink in `argv[1]` and
...
let root = "";
/** The "bundle": a real file, standing in for `dist/fg.mjs`. */
let target = "";
...
beforeAll(() => {
  root = realpathSync(makeTempDir("fg-entry-"));
  target = join(root, "dist", "fg.mjs");
  mkdirSync(join(root, "dist"));
  writeFileSync(target, "// stand-in for the bundle\n");
  targetUrl = pathToFileURL(target).href;

  mkdirSync(join(root, "bin"));
  link = join(root, "bin", "fg");
...
describe("isEntry", () => {
  it("true when the module IS the script, reached by its real path", () => {
    // `node dist/fg.mjs` — the case that always worked, and every other integration suite's.
    expect(isEntry(target, targetUrl)).toBe(true);
  });

  it("true through a SYMLINK — the installed shape, and the blocker", () => {
    // `npm install` writes `node_modules/.bin/fg -> …/dist/fg.mjs`; Node hands the symlink to
