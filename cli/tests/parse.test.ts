...
import { FAKE_COMMANDS, alphaCommand, alphaGroup, betaCommand } from "./fixtures.ts";

const parse = (argv: readonly string[]) => parseInvocation(argv, FAKE_COMMANDS);
/** Against the REAL registry — for cases where which flags the shipped commands declare matters. */
...
/**
 * The rule config's two, which arrive by the same channel and with the same caveat as
 * `--ui-kit`: parseable because `parseArgs` is strict, scoped to `--project-report` because that
 * is the only command whose `ArgSpec`s name them, and absent from the documented globals.
 *
 * `--lint` is GONE (U1). It was a second way to say `--format compact`, and the tokenizer no
 * longer knows the word at all — which is what makes it an unknown flag rather than a flag the
 * wrong command was given.
 */
describe("--config / --format", () => {
  it("hands both to the command through flags, and splits the format list", () => {
    const parsed = parseInvocation(
      ["--preport", "/p", "--config", "./fg.config.json", "--format", "compact,html"],
      COMMANDS,
    );
    expect(parsed.kind).toBe("command");
    if (parsed.kind !== "command") return;
    expect(parsed.flags["config"]).toBe("./fg.config.json");
    expect(parsed.flags["format"]).toBe("compact,html");
    expect(parsed.formats).toEqual(["compact", "html"]);
  });

  it("--lint is not a flag any more, in any position (U1)", () => {
    const parsed = parseInvocation(["--preport", "/p", "--lint"], COMMANDS);
    expect(parsed.kind).toBe("error");
    expect(parsed.kind === "error" && parsed.message.ru).toContain("--lint");
    // …and it is not silently accepted on any other command either.
    expect(parseInvocation(["--psvg", "11:10", "--lint"], COMMANDS).kind).toBe("error");
  });

  it("leaves both absent when nobody typed them", () => {
    const parsed = parseInvocation(["--preport", "/p"], COMMANDS);
    expect(parsed.kind === "command" && parsed.flags["config"]).toBeUndefined();
    expect(parsed.kind === "command" && parsed.flags["format"]).toBeUndefined();
    expect(parsed.kind === "command" && parsed.formats).toBeUndefined();
  });

  /**
   * U1, the whole sentence: "comma list allowed, order irrelevant, duplicates ignored. Unknown
   * value → exit 2 naming the accepted values. `--format` is accepted ONLY by `--preport`".
   *
   * The accepted values are the COMMAND's own — read out of its `ArgSpec` by `declaredValues` —
   * so this suite asserts the refusal quotes the same four words the help page prints.
   */
  it("de-duplicates the list and keeps the order the user typed", () => {
    const parsed = parseInvocation(
      ["--preport", "/p", "--format", "sarif, html ,sarif,,html"],
      COMMANDS,
    );
    expect(parsed.kind === "command" && parsed.formats).toEqual(["sarif", "html"]);
  });

  it("an unknown value is exit 2, naming every accepted value", () => {
    const parsed = parseInvocation(["--preport", "/p", "--format", "html,stylish"], COMMANDS);
    expect(parsed.kind).toBe("error");
    if (parsed.kind !== "error") return;
    expect(parsed.message.ru).toContain("stylish");
    for (const value of ["html", "compact", "json", "sarif"]) {
      expect(parsed.message.ru).toContain(value);
    }
    // The refusal knows WHICH command it is about, so the CLI can quote that command's usage.
    expect(parsed.command?.flag).toBe("--project-report");
  });

  /**
   * V5 FINDING #7 — AN EMPTY LIST IS A MALFORMED INVOCATION, not a request for the default.
   *
   * `--format ""` used to write `fg-out/report.html` and exit 1; `--format ",,,"` used to exit 0.
   * Both were silent, and both are the flag being typed with nothing meant by it — U1's answer
   * for a value the command cannot honour is exit 2 with the accepted list.
   */
  it("an EMPTY or blank list is exit 2, with the same message an unknown value gets", () => {
    for (const raw of ["", ",,,", "   ", " , , "]) {
      const parsed = parseInvocation(["--preport", "/p", "--format", raw], COMMANDS);
      expect(`${JSON.stringify(raw)}: ${parsed.kind}`).toBe(`${JSON.stringify(raw)}: error`);
      if (parsed.kind !== "error") continue;
      // The accepted values are quoted, exactly as for `--format stylish`.
      for (const value of ["html", "compact", "json", "sarif"]) {
        expect(parsed.message.ru).toContain(value);
      }
      expect(parsed.message.ru.startsWith("неизвестный формат:")).toBe(true);
      // A spelling with nothing printable in it is shown quoted, so the message names something.
      if (raw.trim() === "") expect(parsed.message.ru).toContain(`"${raw}"`);
      expect(parsed.command?.flag).toBe("--project-report");
    }
  });

  it("every accepted value passes, alone and together", () => {
    const all = ["html", "compact", "json", "sarif"];
    for (const value of all) {
      expect(parseInvocation(["--preport", "/p", "--format", value], COMMANDS).kind).toBe(
        "command",
      );
    }
    const parsed = parseInvocation(["--preport", "/p", "--format", all.join(",")], COMMANDS);
    expect(parsed.kind === "command" && parsed.formats).toEqual(all);
  });

  /** The values are READ from the command, never from a list in the parser. */
  it("declaredValues reads the accepted set off the ArgSpec that documents it", () => {
    const preport = COMMANDS.find((command) => command.alias === "--preport");
    expect(preport).toBeDefined();
    expect([...declaredValues(preport as CliCommand, "--format")]).toEqual([
      "html",
      "compact",
      "json",
      "sarif",
    ]);
    // A placeholder is not an enumeration: `--config <file>` promises no values, and a command
    // that promises none is not validated at all.
    expect(declaredValues(preport as CliCommand, "--config")).toEqual([]);
    expect(declaredValues(preport as CliCommand, "--nothing")).toEqual([]);
  });

  it("--config and --format without a value are flag-usage errors", () => {
    expect(parseInvocation(["--preport", "/p", "--config"], COMMANDS).kind).toBe("error");
    expect(parseInvocation(["--preport", "/p", "--format"], COMMANDS).kind).toBe("error");
  });

  it("refuses both on a command that does not declare them", () => {
    for (const flag of [
      ["--config", "c.json"],
      ["--format", "json"],
    ]) {
      const parsed = parseInvocation(["--psvg", "11:10", ...flag], COMMANDS);
      expect(parsed.kind).toBe("error");
      expect(parsed.kind === "error" && parsed.message.ru).toContain(flag[0] ?? "");
    }
  });

  it("neither is printed among the global options", () => {
    for (const name of ["config", "format"]) {
      expect([...HELP_GLOBAL_ORDER]).not.toContain(name);
    }
  });
});

/** `--init-config` and its alias, the third command this feature package registers. */
describe("--init-config", () => {
  it("both spellings resolve to the same command, with -o and --ui-kit accepted", () => {
    for (const spelling of ["--init-config", "--iconf"]) {
      const parsed = parseInvocation([spelling, "-o", "./c.json", "--ui-kit", "eds"], COMMANDS);
      expect(parsed.kind).toBe("command");
      if (parsed.kind !== "command") continue;
      expect(parsed.command.flag).toBe("--init-config");
      expect(parsed.out).toBe("./c.json");
      expect(parsed.flags["ui-kit"]).toBe("eds");
    }
  });

  it("refuses --format, which belongs to --project-report", () => {
    const parsed = parseInvocation(["--init-config", "--format", "json"], COMMANDS);
    expect(parsed.kind).toBe("error");
    expect(parsed.kind === "error" && parsed.message.ru).toContain("--format");
  });
});
 ...
 * `FG_KITS_DIR` and never created `/tmp/x`, because `parseArgs`'s options table is shared by
...
describe("scoping is DERIVED from each command's declared args", () => {
  ...
  it("reads a command's flags out of its ArgSpec names, short spelling included", () => {
    expect([...declaredOptions(declaring, options)].toSorted()).toEqual(["out", "token"]);
  });
