 ...
 * checks: that `npm i @smart-tools/frontend-guard` produces a `fg` that WORKS. Not that the
 ...
 * (`node_modules/.bin/fg` -> `../@smart-tools/frontend-guard/dist/fg.mjs`). Node then sets
 ...
 * this package: `run()` was never called, and `fg --version` exited 0 having printed nothing at
 * all. A silent no-op, shipped, with a green suite behind it.
 *
 * The suite was green because every other integration test spawns `node <path>/dist/fg.mjs`,
...
let scratch = "";
/** `<prefix>/node_modules/.bin/fg` — the symlink, and the only thing this file spawns. */
let installedBin = "";
/** Where that symlink points, i.e. the installed `dist/fg.mjs`. */
let installedReal = "";
/** A private copy of the `plain-css` fixture, so a written report never lands in the worktree. */
let project = "";

beforeAll(async () => {
  scratch = makeTempDir("fg-installed-");
  ...
  installedBin = join(prefix, "node_modules", ".bin", "fg");
  installedReal = join(prefix, "node_modules", "@smart-tools", "frontend-guard", "dist", "fg.mjs");
...
describe("the installed layout", () => {
  it("materialises `fg` as a SYMLINK, which is the whole reason this file exists", () => {
...
describe("running the symlinked bin", () => {
  ...
  it("`--help` prints the whole help text and exits 0", async () => {
    ...
    // The TABLE shows the short alias (U8); the long spelling is on the per-command page.
    expect(result.stdout).toContain("--preport");
    expect(result.stdout).toContain(declaredVersion);
    expect(result.stdout.length).toBeGreaterThan(800);
  ...
  it("`--project-report` does the real work and writes a real report", async () => {
    ...
    // U2: this fixture holds five visible errors, so the run reports them and exits 1 — the
    // report is written either way, which is what the assertions below are about.
    ...
    // stdout carries the absolute path of the file it wrote — the one output shape every
    // command shares (`packages/cli-kit/src/out.ts`'s `emitPaths`, U3: paths only, and only
    // into a pipe, which this child's stdout is). Asserted as CONTAINS here because what this
    ...
    // ...and the terminal UI put its summary block on the other stream, where it belongs.
