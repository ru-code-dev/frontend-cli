 ...
 *
 * Concretely, adding `fg-figma` tomorrow is:
 *
 *   import { figmaCommands } from "@smart-tools/fg-figma";        // +1 line
...
} from "@smart-tools/fg-project-report";

/**
 * THE REGISTRY. One flat array, concatenated from the feature packages.
 *
 * `fg-project-report` is the second feature package, and its arrival is the cost test above
 ...
 * `initConfigCommands` is the THIRD command on that same import — `--init-config`/`--iconf`,
 * which writes `fg.config.json` and its schema — and it cost one more spread and nothing else.
 *
 ...
 */
// THE ORDER INSIDE A GROUP IS THIS ARRAY'S — `helpText` prints exactly what it holds — and
// design §2.7 fixes it for «Анализ проекта»: `--preport`, then `--iconf`, then `--pkit`. The
// three read as a sequence in that order (analyse a project → write the config that tunes the
// analysis → regenerate the corpus the analysis measures against), and the page shipped them
// `--preport, --pkit, --iconf` (V5 finding #13).
...
/**
 * THE RESOLVED SETTINGS A COMMAND RUNS AGAINST — `fg-pixso`'s own type, IMPORTED rather than
 * restated (`packages/fg-pixso/src/runtime.ts:44-48`).
 *
 ...
 * `packages/fg-pixso/src/runtime.ts:11-20`. `cli/src/settings.ts` owns that decision and its
 * reasoning.
 */
export type { PixsoRuntime } from "@smart-tools/fg-pixso";
