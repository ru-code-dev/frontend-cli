...
const assertSorted = (value: unknown, path: string): void => {
  ...
  if (typeof value === "object" && value !== null) {
    ...
    expect(keys, `keys of ${path}`).toEqual([...keys].toSorted());
    for (const key of keys) assertSorted((value as Record<string, unknown>)[key], `${path}.${key}`);
  }
};
...
describe("determinism", () => {
  ...
  it("contains no absolute path from the machine it ran on", () => {
    const json = stableStringify(extractFixture("kit-a"));
    expect(json).not.toContain("/packages/fg-kit-extract/tests/fixtures");
  });
});
