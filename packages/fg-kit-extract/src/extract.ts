...
function resolveRenders(builder: Builder): void {
  ...
  for (const key of [...builder.components.keys()].toSorted()) {
    const pending = builder.components.get(key)!;
...
export function extractKit(options: ExtractOptions): KitExtract {
  ...
  for (const pkg of kit.packages) {
    ...
    for (const name of [...exported.keys()].toSorted()) {
  ...
  const kitFiles = project
    ...
    .toSorted((a, b) => (a.getFilePath() < b.getFilePath() ? -1 : 1));
  ...
  for (const key of [...builder.types.keys()].toSorted()) types[key] = builder.types.get(key)!;
