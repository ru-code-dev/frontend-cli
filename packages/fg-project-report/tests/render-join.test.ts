 ...
 *
 * It reads `@smart-tools/fg-analyzer-report`'s BUILT `dist/` (the package's exports map points
 * there, and the megabyte template only exists after `pnpm build` — the template lives in
 * `dist/index.mjs`, never in `src/`, `packages/fg-analyzer-report/src/template.ts` holding only
 * a placeholder). The same assumption every suite in that package makes
 * (`packages/fg-analyzer-report/tests/template.test.ts`).
 */
...
function embedded(html: string): Record<string, unknown> {
  // The slot's exact spelling is the renderer's own (`packages/fg-analyzer-report/src/render.ts:26`).
...
describe("the written file is the real report", () => {
  it("is one self-contained page carrying the payload the engine result produced", async () => {
    ...
    // Exit 1 is U2 over the fixture's one visible error; the FILE is what this suite is about.
    expect(await command?.run(capture({ cwd: dir, source: "app", out }).ctx)).toBe(1);
  ...
  it("the payload survives the substitution byte for byte", async () => {
    ...
    // (`packages/fg-analyzer-report/src/render.ts`).
