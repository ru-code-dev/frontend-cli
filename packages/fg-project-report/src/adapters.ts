 ...
 * what that object teaches it (`packages/fg-analyzer-engine/src/adapter.ts:1-27`). Somebody
 ...
 *     that *are* the kit" (`packages/fg-eds-adapter/src/index.ts:105`). Exact names, not
...
import type { CorpusProvenance, CorpusWarning, EdsArtifacts } from "@smart-tools/fg-eds-adapter";
import {
  createEdsAdapter,
  EDS_PROFILES,
  edsAdapter,
 ...
 * read `packages/fg-eds-adapter/package.json` in the BUILD process and define this identifier
 ...
 */
declare const __FG_EDS_ADAPTER_VERSION__: string | undefined;
 ...
 */
/**
 * A corpus loader for one registered adapter.
 *
 * Written once because the two entries differ only in which adapter and which profile they
 * rebuild over — the precedence (a valid corpus wins, anything else falls back to the embedded
 * snapshot plus a warning) is the same decision for every design system, and stating it twice is
 * how the second one ends up subtly different.
 */
const corpusResolver =
  (
    embedded: KitAdapter,
    embeddedVersion: string | null,
    profile: (typeof EDS_PROFILES)[keyof typeof EDS_PROFILES],
  ) =>
  async (env: NodeJS.ProcessEnv): Promise<AdapterResolution> => {
    const loaded = await loadCorpus({ kit: profile.corpusDir, env });
    if (loaded.corpus === null || loaded.provenance === null) {
      return {
        adapter: embedded,
        provenance: { kind: "embedded", version: embeddedVersion },
    ...
    return {
      ...
      adapter: createEdsAdapter(loaded.corpus as unknown as EdsArtifacts, profile),
...
export const ADAPTERS: readonly AdapterEntry[] = [
  {
    name: edsAdapter.id,
    version:
      typeof __FG_EDS_ADAPTER_VERSION__ === "string" ? __FG_EDS_ADAPTER_VERSION__ : "0.0.0-dev",
    adapter: edsAdapter,
    /**
     ...
     * (`packages/fg-eds-adapter/src/corpus.ts` header). So this function has no failure branch
     * either — the worst case is the embedded snapshot and a sentence, which is what a run with
     * no `~/.fg/kits/eds/` has always done.
     *
     ...
     * load and shared.
     */
    resolve: corpusResolver(edsAdapter, EMBEDDED_VERSION, EDS_PROFILES.eds),
  },
  {
    name: eds2Adapter.id,
    version:
      typeof __FG_EDS_ADAPTER_VERSION__ === "string" ? __FG_EDS_ADAPTER_VERSION__ : "0.0.0-dev",
    adapter: eds2Adapter,
    resolve: corpusResolver(eds2Adapter, EMBEDDED_VERSION_V2, EDS_PROFILES.eds2),
  },
];

/**
 * `--ui-kit eds` on a project that is actually on EDS 2.x.
 *
 * The two design systems share a name, a scope and a vendor, and `eds` is the spelling every
 * existing script and every existing habit already contains. Refusing it on a v2 project would
 * be technically correct and useless: the user asked for "the EDS adapter" and there is exactly
 * one EDS in their manifest. So the request is REDIRECTED to the version their dependencies
 * declare, and the redirect is reported rather than performed silently — a report measured
 * against a design system the user did not name is worse than a refusal, and only the note makes
 * the difference visible.
 *
 * Only in this direction: `--ui-kit eds2` on a v1 project is not redirected, because `eds2` is
 * a NEW spelling that nobody types by habit. Someone who types it means it.
 */
const REDIRECTS: readonly { readonly from: string; readonly to: string }[] = [
  { from: EDS_PROFILES.eds.id, to: EDS_PROFILES.eds2.id },
];

/**
 * `true` when a `--ui-kit <name>` may end up measuring against a DIFFERENT design system.
 *
 * Read by the report command to decide when it may print its header. The header names the
 * snapshot the numbers came from, and for a redirectable name that snapshot is not known until
 * the project's manifest has been read — so the one line of output that must never need a
 * correction waits for the fact instead of guessing it (V6 audit finding #3). Derived from
 * {@link REDIRECTS} rather than restated, so a second redirect rule cannot leave a second
 * lying header behind it.
 */
export const isRedirectable = (name: string): boolean =>
  REDIRECTS.some((rule) => rule.from === name);

/**
 * Applies {@link REDIRECTS} to a choice the user made by name.
 *
 * Exported through {@link redirectedChoice} because the command answers `--ui-kit` BEFORE the
 * project is on disk — a misspelled name must be a line to retype, not something discovered
 * after a clone — and the redirect can only be decided once the manifest is readable. So the
 * name is validated early and the version is settled here, at the first moment the answer
 * exists.
 */
const applyRedirect = (
  ...
  if (choice.kind !== "adapter" || choice.how !== "flag") return choice;

  const redirect = REDIRECTS.find((rule) => rule.from === choice.entry.name);
  const target = adapters.find((entry) => entry.name === redirect?.to);

  // BOTH halves are required. The project must declare the target's packages, AND it must not
  // declare the requested one's — a monorepo mid-migration that has both is a project where the
  // user's own word is the better answer.
  return target !== undefined &&
    target.adapter.kitPackages.some((name) => dependencies.has(name)) &&
    !choice.entry.adapter.kitPackages.some((name) => dependencies.has(name))
    ? { kind: "adapter", entry: target, how: "redirect", requested: choice.entry.name }
    : choice;
};

/** {@link applyRedirect}, over a project directory the caller has on disk. */
export const redirectedChoice = async (
  choice: AdapterChoice,
  dir: string,
  adapters: readonly AdapterEntry[] = ADAPTERS,
): Promise<AdapterChoice> => applyRedirect(choice, await declaredDependencies(dir), adapters);
...
export type AdapterChoice =
  | {
      ...
      /** `redirect` = the user named a kit, and their manifest named a different version of it. */
      readonly how: "flag" | "autodetect" | "redirect";
      /** The spelling the user typed, when it was redirected. Absent otherwise. */
...
export async function selectAdapter(options: SelectAdapterOptions): Promise<AdapterChoice> {
  ...
  if (requested !== undefined && requested !== "") {
    return applyRedirect(requestedAdapter(requested, adapters), dependencies, adapters);
  }
