 ...
 * ── THE DEFAULTS ────────────────────────────────────────────────────────────────────────────
 *
 *   `--psvg`    → `<cwd>/fg-out/pixso/<name>.svg`
 *   `--phtml`   → `<cwd>/fg-out/pixso/<name>.html`
 *   `--pprompt` → `<cwd>/fg-out/pixso/<name>.md`
 *   `--passets` → `<cwd>/fg-out/pixso/<name>/` + the four `card.*` files
 *
...
import type { CommandContext } from "@smart-tools/fg-cli-kit";
import { FG_OUT_DIR, safeSegment } from "@smart-tools/fg-cli-kit";
...
/** The subdirectory of `fg-out/` this package owns. Every pixso artifact lands under it, so a
...
export const DEFAULT_DIR = `${FG_OUT_DIR}/${PIXSO_OUT_DIR}`;
 ...
 *                    guid (`packages/fg-pixso/src/routing.ts`'s `isDesignLink` draws the same
...
export function assetsTarget(ctx: CommandContext, source: string): string {
  if (ctx.out !== undefined && ctx.out !== "") return resolve(ctx.cwd, ctx.out);
  return resolve(ctx.cwd, FG_OUT_DIR, PIXSO_OUT_DIR, designName(source));
}
