...
import { createPixsoCommands, PIXSO_LINK, resolveRoute } from "../src/index.ts";
 ...
 * (`packages/fg-pixso/tests/fixtures/scratch.ts`).
 */
let cwd = "";
beforeEach(() => {
  cwd = scratch();
...
describe("a source that cannot be routed — exit 2, in the language in play", () => {
  for (const flag of FLAGS) {
    it(`${flag} with no source: exit 2, and the message names BOTH accepted forms`, async () => {
      // U3: the refusal is the UI's block, printed once, and `ctx.stderr` is never written.
      const ru = makeContext({ cwd, lang: "ru" });
      expect(await commandFor(flag).run(ru.ctx)).toBe(2);
      expect(ru.out).toEqual([]);
      expect(ru.err).toEqual([]);
      const ruMessage = ru.ui.at(-1) ?? "";
      expect(ruMessage.startsWith("fail:")).toBe(true);
      // The link form is named with the PLACEHOLDER rather than a scheme: the owner's rule is
      // that nowhere a link is SHOWN rather than used does a URL appear, and the refusal is
      // read by exactly the person the help page's `fg --psvg <pixso-link>` example is for.
      expect(ruMessage).toContain(PIXSO_LINK);
      expect(ruMessage).toContain("guid");
      expect(ruMessage).toMatch(/[а-яё]/i);

      const en = makeContext({ cwd, lang: "en" });
      expect(await commandFor(flag).run(en.ctx)).toBe(2);
      const enMessage = en.ui.at(-1) ?? "";
      expect(enMessage).toContain(PIXSO_LINK);
      expect(enMessage).toContain("guid");
      expect(enMessage).not.toMatch(/[а-яё]/i);
      // Two languages, one message — never the same bytes.
      expect(enMessage).not.toBe(ruMessage);
    });

    it(`${flag} with a design link but no token: exit 2, and all three fixes are named`, async () => {
      for (const lang of ["ru", "en"] as const) {
        const { ctx, out, err, ui } = makeContext({
          cwd,
          source: DESIGN_URL,
          lang,
          env: {},
  ...
  it("an empty token is an absent token, not a token of length zero", async () => {
    ...
    expect(ui.at(-1)).toContain("--token");
  });
