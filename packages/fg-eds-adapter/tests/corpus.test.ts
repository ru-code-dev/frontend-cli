 ...
 * **there is no throwing path**. A user's `~/.fg/kits/eds/` can be truncated, hand-edited,
 ...
 *
 * Every test writes into a temp `FG_KITS_DIR`. `writeCorpus` is exercised for real rather than
...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), "fg-corpus-test-"));
...
describe("where a corpus lives", () => {
  it("defaults to ~/.fg/kits and is moved wholesale by FG_KITS_DIR", () => {
    expect(kitsRoot({})).toMatch(/[/\\]\.fg[/\\]kits$/);
  ...
  it("treats an empty override as absent rather than as the empty path", () => {
    // A shell that exports `FG_KITS_DIR=` sets it to `""`. Honouring that literally would put
