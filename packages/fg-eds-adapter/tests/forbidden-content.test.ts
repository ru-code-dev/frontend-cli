...
describe("forbidden content", () => {
  it("scans the sources, the embedded artifacts and the fixtures", () => {
    ...
    // Specifically: both embedded snapshots — five files for EDS 1.x under `src/artifacts/`
    // and five for EDS 2.x under `src/artifacts/v2/`. Ten, because a guard that stopped
    // scanning the second design system's artifacts would be a guard with a hole in it.
    const artifacts = allFiles.filter((file) => file.includes(join("src", "artifacts")));
    expect(artifacts.filter((file) => file.endsWith(".json"))).toHaveLength(10);
    expect(
      artifacts.filter((file) => file.includes(join("artifacts", "v2")) && file.endsWith(".json")),
    ).toHaveLength(5);
  });
