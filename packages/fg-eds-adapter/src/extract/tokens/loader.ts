 ...
 * NATIVE BINARY: it cannot be inlined into `dist/fg.mjs`, and this CLI's whole shape is one
 ...
 */
export const resolveModulePath = (fromDir: string, specifier: string): string | null => {
...
export const isBareSpecifier = (specifier: string): boolean => !/^[./]/.test(specifier);
 ...
 */
export const collectExternalImports = (entries: readonly string[]): string[] => {
  ...
  return [...externals].toSorted(compareStrings);
};
...
export const evaluateGraph = (
  ...
  /**
   * Bare specifiers this run is allowed to resolve, and what they evaluate to.
   *
   * ABSENT for EDS 1.x, and that absence is the v1 contract unchanged: `assertSelfContained`
   * has already refused every bare import, so `requireFrom` below throws on one and this
   * parameter is never consulted. EDS 2.x builds its theme with vanilla-extract, so its graph
   * genuinely imports four packages (S1 break V3) and the loader is handed a stub for each —
   * see `extract/v2/theme-loader.ts`, where the stubs live with the semantics they model.
   */
  stubs: ReadonlyMap<string, PlainRecord> = new Map(),
