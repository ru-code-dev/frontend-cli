...
const serialise = (index: MutableIndex): Record<string, string[]> => {
  ...
  for (const key of [...index.keys()].toSorted(compareStrings)) {
    out[key] = [...(index.get(key) ?? [])].toSorted(compareStrings);
  }
...
export const buildReverseIndex = (tokens: readonly TokenDto[]): TokenReverseIndexDto => {
  ...
  return {
    cssVariable: Object.fromEntries([...cssVariable].toSorted(([a], [b]) => compareStrings(a, b))),
