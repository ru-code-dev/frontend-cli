 ...
 * `packages/fg-source/src/resolve.ts:32-36`. And npm is SHELLED OUT TO rather than reimplemented
 ...
 * arrangement `fg-source` uses for a missing git (`packages/fg-source/src/errors.ts`).
 */
...
export const installUpstream = async (options: InstallUpstreamOptions): Promise<string> => {
  ...
  try {
    ...
    // `packages/fg-source/src/resolve.ts:192-195` uses to separate "no git" from "git said no".
