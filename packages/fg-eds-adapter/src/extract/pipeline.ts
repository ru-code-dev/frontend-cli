...
import { detectKitLayout, resolveKitPathsV2 } from "./v2/paths.ts";
import { extractCorpusV2, readKitVersionV2 } from "./v2/pipeline.ts";
...
export interface ExtractKitOptions {
  /** The UI kit checkout — already on disk, resolved by `@smart-tools/fg-source`. */
  ...
  /**
   * Which extraction pipeline to run, from the profile the CLI resolved.
   *
   * DECLARED rather than detected, so `--pkit eds` pointed at an EDS 2.x checkout fails with a
   * sentence instead of quietly producing a v2 corpus under the v1 name — a corpus in the wrong
   * directory is a report measured against the wrong design system. The layout is then VERIFIED
   * against the checkout, which is the check that catches the mistake.
   */
  readonly extractor?: "v1" | "v2" | undefined;
}
...
export const extractKit = async (options: ExtractKitOptions): Promise<ExtractedKit> => {
  ...
  const wanted = options.extractor ?? "v1";
  const layout = detectKitLayout(options.uiKitRoot);

  if (layout !== null && layout !== wanted) {
    throw new ExtractionError(
      `"${options.uiKitRoot}" is an EDS ${layout === "v1" ? "1.x" : "2.x"} checkout, ` +
        `but the ${wanted} extractor was requested. Pass the matching --parse-ui-kit name.`,
    );
  }

  if (wanted === "v2") {
    // EDS 2.x wraps no upstream (S1 break V10), so there is nothing to install and the whole
    // pipeline runs offline against the checkout.
    const v2Paths = resolveKitPathsV2(options.uiKitRoot);
    const corpus = await extractCorpusV2({ paths: v2Paths, onStage: announce });

    return {
      corpus,
      version: await readKitVersionV2(v2Paths.uiKitRoot),
      commit: await readCommitSha(v2Paths.uiKitRoot),
      // The a11y evidence IS built for v2 — from the kit's own sources. What is absent is an
      // upstream, and that is reported as the adapter's `no-upstream` limitation, not here.
      upstreamAvailable: corpus["kit-a11y"].meta.upstreamAvailable,
    };
  }
