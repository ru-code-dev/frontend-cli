...
import { ALL_TOKEN_TIERS, THEME_MODES } from "../tokens/tiers.ts";
...
export const tokenTierSchema = z.enum(ALL_TOKEN_TIERS);
...
export const tokenSchema = z.object({
  ...
  /**
   * JS member path a consumer writes to reach this token, e.g. `themeTokens.edsSys.Background.
   * backAccent`. `null` when the design system publishes no such object.
   *
   * OPTIONAL, never `null`-filled, and the distinction is the byte-identity law: zod rebuilds a
   * parsed object in SCHEMA order, so a required field writes its key into every token of the
   * 1.x artifact and breaks `tests/parse-ui-kit.integration.test.ts`, which compares a fresh
   * v1 extraction against the embedded snapshot byte for byte. Absent and `null` read the same
   * to a consumer: `token.jsPath ?? null`.
   */
  jsPath: z.string().nullable().optional(),
  /**
   * `true` when the kit's own sources mark this token key `@deprecated`.
   *
   * 28 keys in `edsSys` carry the tag (S1 break V15). Recorded because it is a fact about the
   * design system that the extraction can see; no rule reads it yet, and inventing one here
   * would be a rule nobody asked for hidden inside an extractor.
   */
...
export const tokensArtifactSchema = z.object({
  ...
  meta: z.object({
     ...
     *
     * EDS 1.x has one — `var(--sds-eng-…)` — and omits the field; EDS 2.x publishes both a
     * custom property and a member path for every token, and its consumers write the member
     * path (S1 §4). Optional for the byte-identity reason `jsPath` is optional.
     */
    tokenReference: z.enum(["css-var", "js-path"]).optional(),
    /** The exported object every `jsPath` is rooted in, e.g. `themeTokens`. */
    jsPathRoot: z.string().optional(),
    /** How many token keys the kit marks `@deprecated`. */
    deprecatedTokens: z.number().int().nonnegative().optional(),
    counts: z.object({
      total: z.number().int().nonnegative(),
      /**
       * PARTIAL over the tier vocabulary, because the vocabulary is the union of BOTH profiles.
       *
       * A `z.record` keyed by an enum is exhaustive in zod 4 — every member must be present —
       * and EDS 1.x emits three of the five tier names (`ref`, `sys`, `comp`) while EDS 2.x
       * emits all five under different meanings. Requiring the union would have forced two zero
       * keys into the 1.x artifact and broken its byte-identity; the count block says which
       * tiers this kit HAS, which is the honest shape.
       */
      byTier: z.partialRecord(tokenTierSchema, z.number().int().nonnegative()),
