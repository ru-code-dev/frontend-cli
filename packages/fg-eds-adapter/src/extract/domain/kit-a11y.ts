...
export const kitPatternSchema = z.object({
  ...
  matchedBy: z.enum([
    ...
    "name",
    /**
     * The evidence is the KIT'S OWN source, not an upstream's.
     *
     * EDS 2.x wraps nothing (S1 break V10) — the accessibility behaviour a report may
     * recommend is in `packages/{base,kit}/src/components` itself. A third value rather than
     * reusing `name`, because a reader has to be able to tell "we found the upstream by
     * guessing its name" from "there is no upstream and this is the kit".
     */
    "own-source",
  ]),
