...
const aggregateExternalReExports = (stars: readonly UnresolvedStar[]): ExternalReExportDto[] => {
  ...
  return [...byPackage]
    .map(([packageName, value]) => ({
      ...
      names: [],
      resolved: false,
      reExportedFrom: [...value.from].toSorted(compareStrings),
    }))
    .toSorted((a, b) => compareStrings(a.package, b.package));
};
...
export const extractComponents = async (paths: KitPaths): Promise<ComponentsArtifact> => {
  ...
  const publicSymbols: PublicSymbolDto[] = [...publicSymbolsByName.values()].toSorted((left, right) =>
