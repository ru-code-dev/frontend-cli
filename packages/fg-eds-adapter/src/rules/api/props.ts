 ...
 *
 * ## Why `api.deprecated` also reads imports under some kits
 *
 * The rule was written against a kit that deprecates COMPONENTS, which are visible as JSX. EDS
 * 2.x's public deprecations are two HOOKS — `useFieldSizing` and `useLocalStorage`
 * (`../ui-kit-eds-ce-2/packages/base/src/hooks/{useFieldSizing,useLocalStorage}/…`) — and a hook
 * never appears as an element. A JSX-only rule therefore reports a clean bill of health for a
 * project that calls a deprecated hook on every screen.
 *
 * So the import pass exists, and it is PROFILE-GATED (`KitProfile.deprecatedImports`) rather
 * than switched on for everyone: turning it on for EDS 1.x would change findings this package is
 * required to keep byte-identical with the tool it was ported from. The binding is reported at
 * the import, which is the one place a rename has to happen and the one place the reader can see
 * the whole set of names in play.
 */
...
export const invalidPropRule = ({ kit }: KitContext): Rule => ({
  ...
  label: { ru: "Несуществующее значение пропа", en: "Prop value that does not exist" },
  severity: "error",
...
export const deprecatedApiRule = ({ kit }: KitContext): Rule => ({
  ...
  label: { ru: "Устаревший API", en: "Deprecated API" },
  severity: "warning",
  run: (context: RuleContext): RawFinding[] => {
    const findings: RawFinding[] = [];
    const reported = new Set<string>();

    /** Every deprecated kit symbol used here, in source order: elements, then imports. */
    const uses: {
      symbol: string;
      file: string;
      line: number;
    ...
    for (const element of context.observations.jsxElements) {
      if (element.kitComponent === null) continue;
      const note = kit.deprecationOf(element.kitComponent);
      if (note === undefined) continue;
      uses.push({
        symbol: element.kitComponent,
        file: element.file,
        line: element.line,
        column: element.column,
        note,
    ...
    if (kit.profile.deprecatedImports) {
      const kitPackages = new Set(kit.profile.packages);
      for (const record of context.observations.imports) {
        // Only the kit's own packages: a project is free to have a `useLocalStorage` of its own,
        // and the deprecation belongs to the kit's.
        const packageName = record.specifier.split("/").slice(0, 2).join("/");
        if (!kitPackages.has(packageName) && !kitPackages.has(record.specifier)) continue;

        for (const binding of record.names) {
          const note = kit.deprecationOf(binding.imported);
          if (note === undefined) continue;
          uses.push({
    ...
    for (const use of uses) {
      ...
      const replacement =
        ...
        // A BACKTICKED name, which is how EDS 2.x writes them: "Используйте хук `useStorage`".
        // A hook's name starts lowercase, so the pattern above — anchored on a capital because
        // it was written for component names — cannot see it. Tried only when that one found
        // nothing, and only inside backticks, so no note it already answers changes answer:
        // none of the 1.x notes that fall through to here contains a backtick.
        /`([A-Za-z_$][\w$]*)`/.exec(note ?? "")?.[1] ??
        null;
