import type { Limitation, RawFinding, Rule, RuleContext } from "@smart-tools/fg-analyzer-engine";
...
export const patternKeyboardRule = ({ a11y }: KitContext): Rule => ({
  ...
  label: { ru: "Виджет недоступен с клавиатуры", en: "Widget unreachable from the keyboard" },
  ...
  subkindLabels: {
    noHandler: { ru: "обработчика клавиш нет", en: "no key handler at all" },
    handlerUnreadable: {
      ru: "обработчик объявлен отдельно",
      en: "the handler is declared elsewhere",
    },
  },
  // A widget with SOME handler is a warning; one with none is an error.
  severity: "mixed",
