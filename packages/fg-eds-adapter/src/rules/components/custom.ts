...
export const customComponentRule = ({ knowledge }: KitContext): Rule => ({
  ...
  label: { ru: "Компонент, который кит уже умеет", en: "Component the kit already has" },
  // One rule, three verdicts on the same scale — and a user who wants the certainties without
  // the maybes must be able to keep `component.fork` and silence `component.ambiguous`.
  severity: "mixed",
  emits: [
    {
      id: "component.fork",
      description: "Разошедшаяся копия компонента кита: то же имя и тот же API",
      label: { ru: "Разошедшаяся копия компонента кита", en: "Diverged copy of a kit component" },
      severity: "warning",
    },
    {
      id: "component.custom",
      description: "Локальный компонент повторяет компонент кита",
      label: { ru: "Компонент, который кит уже умеет", en: "Component the kit already has" },
      severity: "warning",
    },
    {
      id: "component.ambiguous",
      description: "Локальный компонент похож на компонент кита, но не настолько, чтобы утверждать",
      label: { ru: "Похож на компонент кита — проверить", en: "Looks like a kit component" },
      severity: "info",
    },
  ],
  run: (context) => {
...
export const novelComponentRule = ({ knowledge }: KitContext): Rule => ({
  ...
  label: { ru: "Кандидат в дизайн-систему", en: "Candidate for the design system" },
  severity: "candidate",
  // This rule owns the clustering, so it also emits `component.duplicate` — the id the
  // engine's split-out copy uses and which `replaces` (`src/index.ts`) hands over to it here.
  // Both are promotion signals for the kit team, hence both `candidate`.
  emits: [
    {
      id: "component.novel",
      description: "Переиспользуемый компонент без аналога в ките — кандидат в дизайн-систему",
    ...
    },
    {
      ...
      label: { ru: "Скопирован внутри проекта", en: "Copy-pasted inside the project" },
      severity: "candidate",
    },
  ],
  run: (context) => {
