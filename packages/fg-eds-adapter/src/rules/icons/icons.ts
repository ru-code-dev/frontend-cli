...
export const inlineSvgRule = ({ icons }: KitContext): Rule => ({
  ...
  label: { ru: "Инлайновый SVG вместо иконки кита", en: "Inline SVG instead of a kit icon" },
  ...
  subkindLabels: {
    "kit-icon": { ru: "ровно эта иконка есть в ките", en: "this exact icon is in the kit" },
    "no-match": { ru: "нет в ките — кандидат в набор", en: "not in the kit — a candidate for it" },
  },
  // kit-icon -> warning (the kit already has it), no-match -> candidate (it does not).
  severity: "mixed",
...
export const foreignSvgFileRule = ({ icons }: KitContext): Rule => ({
  ...
  label: { ru: "SVG-файл мимо набора иконок", en: "SVG file outside the icon set" },
  // The shades this rule grades, named — design §2.4 (V5 finding #6). `ru` is the dashboard's
  // own `SUBKIND_LABEL`, pinned by the parity test; `en` is authored here.
  subkindLabels: {
    "kit-icon": { ru: "ровно эта иконка есть в ките", en: "this exact icon is in the kit" },
