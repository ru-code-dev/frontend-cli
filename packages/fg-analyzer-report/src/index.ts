/**
 * `@smart-tools/fg-analyzer-report` — the analysis, as one HTML file.
 *
 ...
 *
 * RC added the rule config (`WORKFLOW/features/rule-config/plans/rc-design.md` §5): the payload
 * carries every RAW finding plus the config the summary was computed under and the catalog of
 * every rule that could have fired. The dashboard applies the config itself and recounts live,
 * so a reader can switch a category back on without regenerating anything — which is only
 * honest because the data never left. `payloadOf` defaults both fields, so a caller that never
 * heard of the feature produces the report it always produced.
 */
...
export { countFindings, DEFAULT_RULE_CONFIG, payloadOf } from "./payload.ts";
