...
export interface Expected {
  ...
  /** @see `dashboard/src/contract.ts`'s `Expected.jsPath`. Absent for a css-var-only kit. */
  /**
   * Spelled `| undefined` on purpose: `exactOptionalPropertyTypes` is on
   * (`tsconfig.base.json`), and it rejects a zod-parsed optional — whose type includes
   * `undefined` — against a bare `?:`. The same spelling every optional in this repo uses.
   */
  readonly jsPath?: string | null | undefined;
...
/**
 * The reader's view over the rules — design `rc-design.md` §3, mirrored structurally.
 *
 * `off` hides a finding, `on` keeps the severity the rule itself assigned (which for
 * `a11y.lint` varies per sub-rule, so `on` is not a synonym for any single severity), and a
 * severity forces one. Declared here rather than imported from
 * `packages/fg-analyzer-engine/src/config/types.ts:42` for the reason the whole file exists:
 * the two trees compile under different tsconfigs, and the pair is kept honest by the parity
 * suite (`tests/rule-config-parity.test.ts`) rather than by a shared import.
 */
export type RuleLevel = "off" | "on" | Severity;

/**
 * The RESOLVED config the payload embeds — engine `src/config/types.ts:75`.
 *
 * `source` is what the dashboard's rule panel puts in its header: a reader who sees rules
 * switched off is owed the name of the file that switched them off.
 */
...
/**
 * One controllable thing — engine `src/config/types.ts:109`.
 *
 * The dashboard's panel is built from this list and nothing else, so a rule the engine gained
 * appears in the panel of the next report without a dashboard change. `origin` is `"engine"`
 * or the adapter's id; typed as `string` because the set is open by construction.
 */
 ...
 * (`packages/fg-analyzer-engine/src/domain/findings.ts:262-303`) and in the dashboard's own
...
export interface ReportPayload {
  ...
  /**
   * THE CONFIG THE RUN WAS SUMMARISED UNDER — the dashboard's DEFAULT, not its ceiling.
   *
   * `findings` above is the RAW set (design D11): every rule always runs and every finding
   * travels. This field is the view the file asked for, so the dashboard opens showing exactly
   * what the console showed — and, because the data is all there, the reader can switch a
   * category back on and see it without regenerating the report (design D10).
   *
   * Always present. A caller that never heard of rule config gets
   * `{ default: "on", categories: {}, rules: {}, source: { kind: "defaults" } }`, which is the
   * identity view and renders the report this package produced before the feature existed.
   */
  readonly ruleConfig: RuleConfig;
  /**
   * Every rule the run could have produced, whether or not it did.
   *
   * The panel needs the rules with ZERO findings just as much as the rest: "0 icon problems"
   * is only readable once you can see that the icon rules ran. Empty when the caller supplies
   * none, and the panel then falls back to the ids the findings themselves carry.
   */
  readonly ruleCatalog: readonly RuleCatalogEntry[];
}
