 ...
 *
 * The analyzer engine (`@smart-tools/fg-analyzer-engine`, brief B2 §3) keeps the
 ...
 * record. B4's reconciliation: `@smart-tools/fg-analyzer-engine` narrows its category enum to
 * the three its ported rules can emit (`packages/fg-analyzer-engine/src/domain/findings.ts:39`
 ...
 * `KitAdapter` is connected (`packages/fg-analyzer-engine/src/domain/findings.ts:262-303`,
...
export interface AnalyzerResult {
  ...
  /**
   * How many findings the run's config hid — `packages/fg-analyzer-engine/src/index.ts:349`.
   *
   * Declared but not read: the engine sets it beside `findings`/`summary`, and a caller
   * handing this package the engine's result verbatim must typecheck. `exactOptionalPropertyTypes`
   * plus the excess-property check on an object literal would otherwise reject the very shape
   * the engine hands back. The payload does not carry it — the dashboard recounts live, so a
   * number frozen at generation time would be the one thing on screen that could go stale.
   */
...
export interface PayloadOptions {
  ...
  readonly project?:
    ...
    | undefined;
  /**
   * The config the summary was computed under, embedded as the dashboard's default view.
   *
   * Supplied by the caller for the same reason as `adapter`: the engine takes the config as an
   * argument and does not put it back into what it returns — the CLI is the one place that
   * knows which file it found and where. Defaults to {@link DEFAULT_RULE_CONFIG}, so a caller
   * that never heard of rule config still produces a valid payload that renders every finding.
   */
  readonly ruleConfig?: RuleConfig | undefined;
  /**
   * Every rule that could have fired, from `ruleCatalog(adapter)`.
   *
   * Defaults to `[]` — a payload with no catalog is a payload whose rule panel lists only the
   * ids its findings carry, which is a smaller panel, not a broken one.
   */
  readonly ruleCatalog?: readonly RuleCatalogEntry[] | undefined;
}

/**
 * The identity view: nothing hidden, nothing re-graded, no file behind it.
 *
 * A structural copy of `packages/fg-analyzer-engine/src/config/types.ts:95`, for the same
 * reason the types above are copies. Frozen for the reason the engine's is: a module-level
 * constant a caller could spread into and mutate through would corrupt every later payload in
 * the process, and the corruption would be invisible until the second report disagreed with
 * the first.
 */
...
/**
 * The five fields the counters actually read.
 *
 * Named, and narrower than {@link EngineFinding}, because {@link countFindings} is transcribed
 * into the dashboard (`dashboard/src/lib/rule-config.ts`) so the browser can recount live when
 * the reader flips a rule. This type is the exact surface the two copies have to agree on, and
 * the parity suite feeds both the same findings.
 */
export type CountableFinding = Pick<
  EngineFinding,
  "rule" | "category" | "severity" | "autoFixable" | "needsAgent"
>;

/** The `summary.findings` block, recomputed from the findings when the engine omits it. */
export const countFindings = (findings: readonly CountableFinding[]): Summary["findings"] => {
...
export function payloadOf(result: AnalyzerResult, options: PayloadOptions = {}): ReportPayload {
  ...
  return {
    ...
    // Both always present, both defaulted rather than omitted: the dashboard reads them on
    // every render, and "absent" would have to mean "render nothing" somewhere in a screen
    // instead of "there was no file", which is the distinction `source.kind` already carries.
    ruleConfig: options.ruleConfig ?? DEFAULT_RULE_CONFIG,
    ruleCatalog: options.ruleCatalog ?? [],
  };
}
