  ...
  return (
    <div className="ds-enter">
      ...
      <footer className="flex flex-wrap items-center gap-1.5 border-t border-border px-4 py-2.5">
        ...
        {/*
          THE SECOND REFERENCE CHANNEL. A design system can publish a token as a custom property,
          as a member path off an imported object, or as both — and the two are not
          interchangeable in a file: a `.css.ts` cannot hold `var(--…)` and a `.scss` cannot hold
          `themeTokens.…`. `value` above already carries the one this kit's consumers write, so
          this button is the OTHER one, offered only when it exists and differs from it. On a
          report from a css-var-only design system the field is absent and nothing renders.
        */}
        {finding.expected?.jsPath != null && finding.expected.jsPath !== finding.expected.value && (
          <CopyButton value={finding.expected.jsPath} label="JS-путь" />
        )}
        {finding.expected?.cssVar != null &&
          `var(${finding.expected.cssVar})` !== finding.expected.value && (
            <CopyButton value={`var(${finding.expected.cssVar})`} label="CSS-переменная" />
          )}
        {hasDiff && (
          <Button
            onClick={() => {
