...
export const OverviewScreen = ({
  ...
  // `""` when the report had no config file — and then NOTHING is rendered, on either of the
  // two places that carry it. A report nobody configured must read exactly as it read before
  // this feature existed (V4 audit finding 2).
  const note = configNote(payload.ruleConfig);
          ...
          {kit !== null && (
            ...
            >
              ...
              <div className="text-center">
                ...
                {/* RC: the one number on this screen that does NOT follow the rule panel —
                    it is computed over the project's files, which the payload does not carry.
                    Said only when a config file decided something; otherwise the card is the
                    card it has always been. */}
                {note !== "" && (
                  <p className="mt-1 max-w-[160px] text-[11px] leading-snug text-faint">{note}</p>
                )}
              </div>
            </div>
          )}
