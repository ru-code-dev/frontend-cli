 ...
 * feature package into `fg-source` so the analyzer, the report command and anything after them
 * acquire trees the same way. Consequently there is no CLI knowledge here: no `CommandContext`,
 * no `Localized`, no exit codes, not even a dependency on `@smart-tools/fg-cli-kit`. Node
...
const DEFAULT_TMP_PREFIX = "fg-source-";
 ...
 * `WORKFLOW/features/hackathon-analys/reports/b1-fg-source.md` calls it out for the owner.
 */
...
export interface ResolveSourceOptions {
  ...
  /** Prefix for the `mkdtemp` directory. Default `"fg-source-"`. */
