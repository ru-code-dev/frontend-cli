 ...
 *
 * WHY NOT `@smart-tools/fg-testkit`, which already has `makeTempDir`
 ...
 * And testkit currently re-exports fixtures from `fg-pixso` by relative path
...
const IDENTITY = [
  "-c",
  "user.email=fg-source@example.invalid",
  "-c",
  "user.name=fg-source tests",
  "-c",
...
export async function buildBareRepo(root: string): Promise<BareRepoFixture> {
  ...
  const headContent = "<main>fg-source fixture</main>\n";
