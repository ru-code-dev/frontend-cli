...
beforeAll(async () => {
  ...
  originRoot = await scratchDir("fg-source-origin-");
...
function privatePrefix(name: string): string {
  return `fg-source-${name}-${process.pid}-${Math.random().toString(36).slice(2, 10)}-`;
}
...
describe("the clone refusals, and the temp directory they must not leave behind", () => {
  ...
  it("no git on PATH → git-not-installed, and still nothing left behind", async () => {
    ...
    const emptyBin = await scratchDir("fg-source-nogit-");
    made.push(emptyBin);
