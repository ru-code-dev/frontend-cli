...
/**
 * Where a style value's token reference points, when it is a reference rather than a literal.
 *
 * Two kinds because there are two ways a design system publishes its tokens, and they are not
 * interchangeable at the syntax level:
 *
 *  - `css-var` — `color: var(--brand-text-primary)`. The reference is a *string inside the
 *    value*, so it survives in `StyleValue.value` and every string-based helper
 *    (`extractValueLiterals`, `isSoleCustomPropertyReference`) already finds it. Recording it
 *    here as well costs nothing and lets one rule read one field for both kinds.
 *  - `js-path` — `color: tokens.sys.color.textPrimary` in a `.css.ts`. The reference is a
 *    MEMBER EXPRESSION. Nothing survives into the CSS text: by the time vanilla-extract has
 *    run, the property holds a hashed custom property nobody wrote. Without this field a
 *    project that references tokens correctly on every line is indistinguishable from one that
 *    references none.
 *
 * `module`/`exportName` are the import the path is rooted in, EXACTLY as written in the
 * source — whatever the design system in front of it calls its own. This package knows no
 * module names and no export names: the collector reads the file's own import list, and
 * deciding whether such a path names a token is the connected kit's job
 * ({@link KitBinding.tokenIdOfReference}).
 */
export const tokenReferenceSchema = z.discriminatedUnion("kind", [
  z.object({
    kind: z.literal("css-var"),
    /** Custom property name including the leading dashes, e.g. `--brand-text-primary`. */
    name: z.string().min(1),
  }),
  z.object({
    kind: z.literal("js-path"),
    /** Module specifier the root identifier was imported from, as authored. */
    module: z.string().min(1),
    /** Name exported by that module, i.e. the `imported` side of the binding. */
    exportName: z.string().min(1),
    /** Member names read off it, in order: `['sys', 'color', 'textPrimary']`. */
    path: z.array(z.string().min(1)),
    /**
     * `true` when the path was SPREAD into a style object rather than assigned to a property:
     * `{ ...tokens.sys.typography.body, color: '#fff' }`.
     *
     * A different fact from a property reference, and conflating them costs a false finding in
     * both directions. The spread supplies a GROUP of declarations — a whole typography set —
     * so a rule that checks whether an object declares `font-size` and `line-height` together
     * must see that the missing half may be arriving here, and must not report the object as
     * partial. What the group expands to is the kit's knowledge, not the collector's: if the
     * binding cannot expand it, the run says so with an `unresolved-token-reference`
     * limitation rather than reporting the object as incomplete.
     */
    spread: z.boolean(),
  }),
]);
...
export const styleValueSchema = z.object({
  ...
  rootCause: z
    ...
    .nullable(),
  /**
   * The token this value references, or `null` when it is a literal.
   *
   * Populated for BOTH reference kinds — a sole `var(--…)` value and a member path off an
   * imported object — so that a rule asking "is this a token reference?" reads one field
   * rather than one field and a regular expression, and so that the token-usage histogram
   * counts a `.css.ts` project at all.
   *
   * `null` where the value is a literal, or where it is a reference the collector refused to
   * guess at: a computed member access is recorded as `dynamic` with an
   * `unresolved-token-reference` limitation beside it, never as a resolved reference.
   */
  reference: tokenReferenceSchema.nullable(),
 ...
 *
 * `@5` was the hackathon's own final version (`ds-analyzer/src/domain/observations.ts:245`);
 * the namespace is this package's because the payload differs by the two kit-shaped fields
 * dropped in `profile.ts` and by `declaration.kitComponentsUsed`. `@6` adds
 * {@link styleValueSchema}'s `reference`, and it is bumped rather than made optional for the
 * reason stated above: a cached `@5` observation set read by a token rule would report a
 * `.css.ts` project as referencing no tokens, which is the silence-that-reads-as-a-pass this
 * engine refuses. `Observations` is not part of `AnalyzerResult`, so no consumer payload moves.
 */
export const OBSERVATIONS_SCHEMA_ID = "fg-analyzer-engine/observations@6";
