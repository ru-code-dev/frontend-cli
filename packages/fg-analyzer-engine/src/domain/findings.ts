...
export const expectedSchema = z.object({
  ...
  value: z.string(),
  /**
   * The token as a JS member path — `tokens.sys.color.textPrimary` — for a design system
   * whose tokens are an imported object rather than CSS custom properties (design E8). The
   * dashboard shows it where `cssVar` is `null`.
   *
   * OPTIONAL, not `null`-filled, and the difference is the whole additivity guarantee. This
   * field sits at the end of the schema, and zod rebuilds a parsed object in SCHEMA ORDER: a
   * required `jsPath` would write `"jsPath": null` into every finding that carries an
   * `expected` at all — the eight engine fixtures under `tests/golden/`, and every EDS parity
   * golden in `packages/fg-eds-adapter/tests/golden/`, which are byte-compared against the
   * tool this engine replaces and which has no such field. Absent is the only spelling of
   * "this kit does not use JS paths" that keeps those bytes.
   *
   * A reader treats absent and `null` alike; `expected.jsPath ?? null` is the whole contract.
   */
...
export const analyzerResultSchema = z.object({
  $schema: z.literal("fg-analyzer-engine/analysis@1"),
  ...
  /**
   * How many findings a `ruleConfig` hid — CONFIG-gated, and last in the schema on purpose.
   *
   * `findings` above is always the RAW set (design D11): the report embeds every finding so
   * the dashboard can switch a rule back on in the browser without re-running the analysis.
   * The number of findings the file's config removed is therefore not derivable from the
   * payload alone, and it is what the console card reports. Present only when
   * `analyzeProject` was given a config, so a run without one serialises exactly as before.
   */
