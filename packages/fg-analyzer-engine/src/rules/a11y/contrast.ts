...
import { tokenColorForReference } from "../../domain/token-reference.ts";
...
const colorOf = (styleValue: StyleValue, context: RuleContext): ColorValue | null => {
  ...
  // A `js-path` reference carries no `var(--…)` in its text at all — the property will hold a
  // hashed custom property that only the kit can resolve — so it is asked structurally.
  const reference = styleValue.reference;
  if (reference !== null && reference.kind === "js-path") {
    const referenced = tokenColorForReference(context.kit, reference);

    return referenced === null ? null : parseColor(referenced);
  }
...
export const textContrastRule: Rule = {
  ...
  label: { ru: "Текст не набирает контраст", en: "Text below the contrast threshold" },
  ...
  subkindLabels: {
    normalText: { ru: "обычный текст, порог 4.5:1", en: "normal text, 4.5:1 threshold" },
    largeText: { ru: "крупный текст, порог 3:1", en: "large text, 3:1 threshold" },
  },
  severity: "mixed",
