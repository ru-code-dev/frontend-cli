...
const owningDeclaration = (
  ...
  return candidates.toSorted((left, right) => right.line - left.line)[0] ?? null;
};
...
export const dialogFocusRule: Rule = {
  ...
  // SHORTENED, and the dashboard's `RULE_LABEL` with it: «Диалог не отпускает и не удерживает
  // фокус» is 41 code points against the ≤ 40 the design budgets a table string (V5 finding
  // #17), and it was the only one of 62 labels past it.
  label: {
    ru: "Диалог не удерживает и не отдаёт фокус",
    en: "Dialog neither traps nor releases focus",
  },
  ...
  subkindLabels: {
    noEscape: { ru: "не закрывается по Escape", en: "does not close on Escape" },
    noFocusTrap: { ru: "не удерживает фокус", en: "does not trap focus" },
  },
  severity: "error",
