...
export const invalidAriaRule: Rule = {
  ...
  label: { ru: "Несуществующая роль или ARIA-атрибут", en: "Unknown role or ARIA attribute" },
  ...
  subkindLabels: {
    unknownRole: { ru: "роли нет в спецификации", en: "the role is not in the specification" },
    abstractRole: { ru: "абстрактная роль", en: "an abstract role" },
    unknownAttribute: {
      ru: "атрибута нет в спецификации",
      en: "the attribute is not in the specification",
    },
    unsupportedAttribute: {
      ru: "роль не поддерживает атрибут",
      en: "the role does not support the attribute",
    },
    prohibitedAttribute: {
      ru: "атрибут запрещён для роли",
      en: "the attribute is prohibited for the role",
    },
  },
  severity: "mixed",
...
export const requiredAriaRule: Rule = {
  ...
  label: { ru: "Роль без обязательного состояния", en: "Role missing a required state" },
  severity: "error",
  run: (context: RuleContext): RawFinding[] => {
    const findings: RawFinding[] = [];
...
export const redundantRoleRule: Rule = {
  ...
  label: { ru: "Роль дублирует семантику тега", en: "Role duplicating the tag semantics" },
  severity: "info",
  run: (context: RuleContext): RawFinding[] => {
    const findings: RawFinding[] = [];
