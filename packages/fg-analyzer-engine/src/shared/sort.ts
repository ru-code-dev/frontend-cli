...
export const sortStrings = (values: Iterable<string>): string[] => [...values].toSorted(compareStrings);

export const compareNumbers = (left: number, right: number): number => left - right;

/** Numbers ascending, materialised. */
export const sortNumbers = (values: Iterable<number>): number[] => [...values].toSorted(compareNumbers);

/** Items by a string key, stable and reproducible. */
export const sortBy = <T>(items: Iterable<T>, keyOf: (item: T) => string): T[] =>
  [...items].toSorted((left, right) => compareStrings(keyOf(left), keyOf(right)));
