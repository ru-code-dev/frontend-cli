...
/**
 * Above this, a code or style file is skipped rather than read and parsed (V6 audit finding
 * #7). `walkProject` matches by extension alone, so a large GENERATED artifact that happens to
 * carry one of {@link CODE_EXTENSIONS} — the product's own 22 MB release bundle at a project
 * root is the case that was found — is walked like any source file and, without this cap, is
 * fully read into a string and handed to `ts-morph`, which is minutes of wall time and gigabytes
 * of RSS for a file with nothing a design-system audit could act on. Ordinary source files are
 * one to a few hundred KB; 1 MiB already generously covers a large generated types file.
 */
export const MAX_PARSEABLE_FILE_BYTES = 1024 * 1024;
...
export const walkProject = (options: WalkOptions): WalkResult => {
  ...
  const visit = (directory: string, matcher: IgnoreMatcher, depth: number): void => {
    ...
    for (const entry of [...entries].toSorted((left, right) => compareStrings(left.name, right.name))) {
  ...
  return {
    ...
    configFiles: configFiles.toSorted(compareStrings),
    ignoredCount,
    symlinks: symlinks.toSorted(compareStrings),
  };
};
