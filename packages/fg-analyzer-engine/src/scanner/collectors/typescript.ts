...
import {
  collectVanillaExtract,
  type ImportedBinding,
  type StyleFactoryModule,
} from "./vanilla-extract.ts";
...
export interface TypeScriptCollectionInput {
  ...
  /**
   * Modules whose named exports produce vanilla-extract style objects, beyond the bare
   * `@vanilla-extract/*` packages this collector already knows.
   *
   * Supplied by the connected kit at run time (`KitBinding.tokenReferenceModel`), never by a
   * constant here. Omitted — which is every adapter-less run — the vanilla-extract pass still
   * runs for files that import vanilla-extract directly, and does nothing for every other
   * file: no existing observation moves.
   */
...
export const collectTypeScript = (input: TypeScriptCollectionInput): TypeScriptCollectionResult => {
  ...
  try {
    ...
    /**
     * Local identifier → the import that bound it, VALUE imports only.
     *
     * The one piece of state a `js-path` token reference needs: `tokens.sys.color.x` is a
     * member expression whose meaning is entirely in where `tokens` came from. Type-only
     * bindings are excluded because they are erased and can never be a value.
     */
    ...
    for (const declaration of sourceFile.getImportDeclarations()) {
      ...
      if (!declaration.isTypeOnly()) {
        for (const named of declaration.getNamedImports()) {
          if (!named.isTypeOnly()) {
            importedBindings.set(named.getAliasNode()?.getText() ?? named.getName(), {
              module: specifier,
        ...
        if (defaultImport !== null) {
          importedBindings.set(defaultImport, { module: specifier, exportName: "default" });
        }
        const namespaceImport = declaration.getNamespaceImport()?.getText() ?? null;
        if (namespaceImport !== null) {
          importedBindings.set(namespaceImport, { module: specifier, exportName: "*" });
        }
      }
    ...
    // --- vanilla-extract ---------------------------------------------------------------
    // Runs before every pass below so that an object already read as a `style()` body is not
    // read a second time by the shape-based pass under a worse name, and so that the literals
    // inside it are not reported again as bare TypeScript values. A file that imports no
    // vanilla-extract and no declared factory returns nothing at all, which is why an
    // adapter-less run over an ordinary React project is unchanged by this stage.
    const vanillaExtract = collectVanillaExtract({
      file,
      sourceFile,
      bindings: importedBindings,
      ...(input.styleFactories === undefined ? {} : { styleFactories: input.styleFactories }),
    });
    styleValues.push(...vanillaExtract.styleValues);
    limitations.push(...vanillaExtract.limitations);
    for (const offset of vanillaExtract.collectedObjects) {
      collectedObjects.add(offset);
    }
    ...
    for (const element of jsxNodes) {
        ...
        if (Node.isStringLiteral(initializer)) {
          ...
          if (PRESENTATIONAL_ATTRIBUTES.has(attributeName)) {
            styleValues.push({
              ...
              important: false,
              dynamic: false,
              reference: customPropertyReference(literal),
              rootCause: null,
              appliedTo: null,
            });
          }
      ...
      jsxElements.push({
        ...
        styleRefs: styleRefs.map((ref) =>
          Object.assign({}, ref, { module: styleModules.get(ref.module) ?? ref.module }),
        ),
        hasInlineStyle,
        file,
    ...
    for (const literal of sourceFile.getDescendantsOfKind(SyntaxKind.StringLiteral)) {
      ...
      styleValues.push({
        ...
        important: false,
        dynamic: false,
        // A `ts-literal` is by construction a colour or a run of lengths (see the two patterns
        // above), so it is never a reference — but the helper decides that, not this comment.
  ...
  return {
    styleValues: styleValues.toSorted(
      (left, right) =>
