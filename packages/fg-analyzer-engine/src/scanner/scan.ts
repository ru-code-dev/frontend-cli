...
import type { StyleFactoryModule } from "./collectors/vanilla-extract.ts";
import {
  aliasSourceForFile,
  mergeAliases,
  readConfigAliases,
...
import { isCodeFile, isStyleFile, MAX_PARSEABLE_FILE_BYTES, walkProject } from "./walk.ts";
...
export interface ScanOptions {
  ...
  readonly kit?: {
    ...
    /**
     * Modules whose exports produce vanilla-extract style objects, from the adapter's
     * `tokenReferenceModel`. Omitted leaves the collector with the bare `@vanilla-extract/*`
     * packages, which is what an adapter-less run has.
     */
...
/**
 * `true` when `path` is over {@link MAX_PARSEABLE_FILE_BYTES} and must be skipped without being
 * read (V6 audit finding #7). A `statSync` failure (a race with a deleted file, say) is not this
 * function's problem — `readFileSafely` right after it produces the existing `parse-error`
 * limitation for that case, so this only ever answers `false` on any doubt.
 */
const isOverSizeCap = (path: string): boolean => {
  try {
    return statSync(path).size > MAX_PARSEABLE_FILE_BYTES;
  } catch {
    return false;
  }
};

const fileTooLargeLimitation = (file: string): Limitation => ({
  file,
  line: null,
  reason: "file-too-large",
  detail: `файл больше ${String(Math.round(MAX_PARSEABLE_FILE_BYTES / 1024))} КиБ — пропущен без разбора`,
});
...
export const scanProject = (options: ScanOptions): ScanResult => {
  ...
  // scope: `_vars.scss` routinely sits outside the folder being audited. An oversized file is
  // skipped here too — the reason it is not read is the same one whether or not it is in scope.
  const stylesheetContents = new Map<string, string>();
  for (const file of walk.files.filter(isStyleFile)) {
    const absolute = fromProjectPath(root, file);
    if (isOverSizeCap(absolute)) {
      continue;
    }
    const content = readFileSafely(absolute);
    if (content !== null) {
  ...
  for (const file of styleFiles) {
    if (isOverSizeCap(fromProjectPath(root, file))) {
      unparseable += 1;
      limitations.push(fileTooLargeLimitation(file));
      fileDone();
      continue;
    }
  ...
  for (const file of codeFiles) {
    ...
    const result = collectTypeScript({
      ...
      ...(options.kit?.styleFactories === undefined
        ? {}
        : { styleFactories: options.kit.styleFactories }),
    });
  ...
  const profile: ProjectProfile = {
    $schema: "fg-analyzer-engine/project-profile@1",
    root,
    scope,
    name: manifest.name,
    ...
    styleSyntaxes: [...syntaxes].toSorted(compareStrings),
    files: {
  ...
  const observations: Observations = {
    ...
    lintMessages: lintMessages.toSorted(
      (left, right) =>
