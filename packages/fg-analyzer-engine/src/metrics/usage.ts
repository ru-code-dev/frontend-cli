...
import { tokenIdForReference } from "../domain/token-reference.ts";
...
export const buildUsage = (
  ...
  const countToken = (file: string, tokenId: string | null): void => {
    if (tokenId === null) {
      return;
    }
    tokenUsage[tokenId] = (tokenUsage[tokenId] ?? 0) + 1;
    tokenRefsByFile.set(file, (tokenRefsByFile.get(file) ?? 0) + 1);
  };
  ...
  for (const styleValue of observations.styleValues) {
    for (const literal of extractValueLiterals(styleValue.value)) {
      ...
      countToken(styleValue.file, kit.tokenIdOf(literal.name));
    }

    // A `js-path` reference has no `var(--…)` in its value for the loop above to find — that
    // is the entire reason `StyleValue.reference` exists (design §2.2/§2.6). Counted here and
    // nowhere else, so a `css-var` reference is not counted twice: `extractValueLiterals`
    // already found it in the value text.
    const reference = styleValue.reference;
    if (reference !== null && reference.kind === "js-path") {
      countToken(styleValue.file, tokenIdForReference(kit, reference));
    }
  }
  ...
  return {
    ...
    foreignComponents: [...foreign.entries()]
      .map(([name, use]) => {
        ...
        const topSource = [...use.sources.entries()].toSorted(
      ...
      .toSorted((left, right) => right.usages - left.usages || compareStrings(left.name, right.name)),
    customComponents,
    elementBreakdown,
