import type { TokenReference } from "../domain/observations.ts";
...
/**
 * The custom property a value is a sole reference to, as a {@link TokenReference}.
 *
 * The structured half of {@link isSoleCustomPropertyReference}, and deliberately the same
 * predicate rather than a second one: a value that reads as a token reference to a rule and a
 * value that is recorded as one in `StyleValue.reference` must be the same set, or the token
 * histogram and the token rules disagree about the same line.
 *
 * A value with a fallback — `var(--x, #fff)` — is NOT a sole reference, exactly as before: the
 * literal in it is a real authored decision and is reported on its own.
 */
export const customPropertyReference = (value: string): TokenReference | null => {
  const name = /^var\(\s*(--[\w-]+)\s*\)$/.exec(value.trim())?.[1];

  return name === undefined ? null : { kind: "css-var", name };
};
